...
namespace Soomla.Levelup {
	...
	public class World : SoomlaEntity<World> {
		...
		/// </summary>
		/// <param name="id">The ID of the to be added. IT need to be unique inside the level, but not for the whole world as it will be created new.</param>
		public void AddLevelScore(string id) {
            Score score = new Score(IdForAutoGeneratedLevelScore(id));
			Scores.Add(score.ID, score);
		}

        /// <summary>
		/// Get the given <c>Score</c> from this <c>World</c>.
		/// </summary>
		/// <param name="id">The ID of the to be added. IT need to be unique inside the level, but not for the whole world as it will be created new.</param>
		public Score GetLevelScore(string id) {
            return Scores[IdForAutoGeneratedLevelScore(id)];
        }
		...
		/// <param name="scoreId">ID of the <c>Score</c> to be decreased.</param>
		/// <param name="amount">Amount to decrease by.</param>
		public void DecLevelScore(string scoreId, double amount) {
            Scores[IdForAutoGeneratedLevelScore(scoreId)].Dec(amount);
        }
        ...
        public void IncLevelScore(string scoreId, double amount) {
            Scores[IdForAutoGeneratedLevelScore(scoreId)].Inc(amount);
        }
        ...
        private string IdForAutoGeneratedLevelScore(string id) {
            return this.ID + "_score_" + id;
        }
